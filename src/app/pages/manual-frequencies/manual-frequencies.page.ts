...
export class ManualFrequenciesPage implements OnDestroy {
  ...
  private async startPlayback() {
    try {
      ...
      this.notificationService.showInfo(
        'Reproduciendo',
        `${this.selectedWaveformLabel} a ${this.frequency}Hz por ${this.duration} minuto(s)`
      );
      ...
      setTimeout(() => {
        if (!this.audioService.getIsPlaying()) {
          ...
          this.notificationService.showSessionComplete(this.frequency, this.duration);
        }
      }, this.duration * 60 * 1000);
      
    } catch (error) {
      console.error('Error playing frequency:', error);
      this.notificationService.showAudioError(
        'Error al reproducir el sonido. Asegúrate de que tu navegador permita la reproducción de audio.'
      );
    }
  }
  ...
  private stopPlayback() {
    ...
    // Mostrar notificación de detención
    this.notificationService.showInfo(
